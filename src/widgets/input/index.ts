...
export default class Input extends AbstractWidget {
	...
	public isValid(value: string,  ruleName: string, ruleValue: any): boolean {
    ...
    public setErrors(error: any): void {
    ...
    protected onChange(e: Event): void {
		...
		public getError(ruleName: string, ruleValue: any): any {
			if (ruleName === 'maxLength' || ruleName === 'minLength') {
				return {[ruleName]: Input.errors[ruleName](ruleValue)};
			} else {
				return {[ruleName]: Input.errors[ruleName]};
			}
		}

		public checkRules(value: string): void {
			if (this.rules) {
				Object.entries(this.rules).map(([ruleName, ruleValue]: [key: string, value: any]) => {
					...
					if (!isValid) {
						error = this.getError(ruleName, ruleValue);
